...
func newUser(db *gorm.DB, opts ...gen.DOOption) user {
	...
	_user.TimeZone = field.NewString(tableName, "time_zone")
	_user.FCMTokens = userHasManyFCMTokens{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("FCMTokens", "models.UserFCMToken"),
		User: struct {
			field.RelationField
			FCMTokens struct {
				field.RelationField
			...
			RelationField: field.NewRelation("FCMTokens.User", "models.User"),
			FCMTokens: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("FCMTokens.User.FCMTokens", "models.UserFCMToken"),
			},
		},
	}
...
type user struct {
	...
	FCMTokens userHasManyFCMTokens
...
func (u *user) updateTableName(table string) *user {
	...
	u.TimeZone = field.NewString(table, "time_zone")
...
func (u *user) fillFieldMap() {
	...
	u.fieldMap["time_zone"] = u.TimeZone
...
type userHasManyFCMTokens struct {
	db *gorm.DB
...
func (a userHasManyFCMTokens) Where(conds ...field.Expr) *userHasManyFCMTokens {
	if len(conds) == 0 {
		return &a
	}
...
func (a userHasManyFCMTokens) WithContext(ctx context.Context) *userHasManyFCMTokens {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a userHasManyFCMTokens) Session(session *gorm.Session) *userHasManyFCMTokens {
	a.db = a.db.Session(session)
	return &a
}

func (a userHasManyFCMTokens) Model(m *models.User) *userHasManyFCMTokensTx {
	return &userHasManyFCMTokensTx{a.db.Model(m).Association(a.Name())}
}

type userHasManyFCMTokensTx struct{ tx *gorm.Association }

func (a userHasManyFCMTokensTx) Find() (result []*models.UserFCMToken, err error) {
	return result, a.tx.Find(&result)
}

func (a userHasManyFCMTokensTx) Append(values ...*models.UserFCMToken) (err error) {
...
func (a userHasManyFCMTokensTx) Replace(values ...*models.UserFCMToken) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyFCMTokensTx) Delete(values ...*models.UserFCMToken) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyFCMTokensTx) Clear() error {
	return a.tx.Clear()
}

func (a userHasManyFCMTokensTx) Count() int64 {
	return a.tx.Count()
}
